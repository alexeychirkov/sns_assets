...
lines.push(`import { SnsSwapLifecycle } from "./types.js";`);
lines.push(``);
lines.push(`/**`);
lines.push(` * Static snapshot of all deployed SNS projects.`);
lines.push(` * Generated: ${new Date(data.fetchedAt).toISOString()}`);
lines.push(` * Total: ${data.projects.length} projects (${data.projects.filter((p) => p.lifecycle === 3).length} committed)`);
lines.push(` */`);
...
for (const p of data.projects) {
  ...
  lines.push(`    swapCanisterId: ${JSON.stringify(p.swapCanisterId || "")},`);
  lines.push(`    tokenSymbol: ${JSON.stringify(p.tokenSymbol)},`);
  lines.push(`    tokenDecimals: ${p.tokenDecimals},`);
  lines.push(`    lifecycle: ${lc} as SnsSwapLifecycle,`);
  if (p.logoDataUrl) {
