...
import {type E8s, type NeuronInfo, NeuronState, Vote} from "@dfinity/nns";
import type {Principal} from "@dfinity/principal";
import type {SnsNeuronId} from "@dfinity/sns";
import {type SnsNervousSystemFunction, type SnsNervousSystemParameters, type SnsNeuron, SnsNeuronPermissionType, type SnsProposalData,} from "@dfinity/sns";
import {fromDefinedNullable, fromNullable, isNullish, nonNullish, uint8ArrayToHexString,} from "@dfinity/utils";
import {HOTKEY_PERMISSIONS} from "src/ui/pages/sns/neuron/sns-neurons.constants";
import {formatNumber} from "src/util/numberFormat";
import {bonusMultiplier, getVotingPower} from "src/util/neuronUtils";

const formatTokenE8s = ({value}: { value: bigint }): string => formatNumber(value, 8);
...
): string => uint8ArrayToHexString(Array.from(subaccount));
...
export const formattedTotalMaturity = (neuron: SnsNeuron): string =>
    formatTokenE8s({
        value: neuronTotalMaturity(neuron)
    });

export const neuronTotalMaturity = (neuron: SnsNeuron): bigint =>
    neuron.maturity_e8s_equivalent +
